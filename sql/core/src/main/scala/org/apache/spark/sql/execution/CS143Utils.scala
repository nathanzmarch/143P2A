    ...
    new Iterator[Row] {
      ...
      def next() = {
        ...
        if (input.hasNext) {
          ...
          if (cache.containsKey(key)) projectedUDF = cache.get(key) else {
            projectedUDF = udfProject(row)
            cache.put(key, udfProject(key))
          }
          Row.fromSeq(preUdfProjection(row) ++ projectedUDF ++ postUdfProjection(row))
        }
        else null
      }
    }
